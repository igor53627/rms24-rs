# Schema40 v3 + CodeStore Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the current value+tag schema40 helpers with the full Plinko v3 account/storage layout and add CodeStore support (bytecode hash dedup) with tests.

**Architecture:** Keep `schema40` as a pure data module (no DB access). Implement Plinko v3 entry layouts (account: balance/nonce/code_id/tag/pad, storage: value/tag), Tag derivation from address/slot, CodeId + CodeStore. All logic is byte-oriented with explicit LE conversions and fixed-size arrays.

**Tech Stack:** Rust 2021, `sha3` (Keccak256), `std::collections::HashMap`.

---

### Task 1: Add constants + Tag + CodeId (v3 semantics)

**Files:**
- Modify: `src/schema40.rs`
- Test: `src/schema40.rs`

**Step 1: Write the failing tests**

Add to `src/schema40.rs` tests:
...
Run: `cargo test schema40::tests::test_entry_sizes`
Expected: FAIL (missing constants/types).

**Step 3: Implement constants + Tag + CodeId**

In `src/schema40.rs`, add:
...
impl CodeId {
    pub fn new(id: u32) -> Self { CodeId(id) }
    pub fn is_eoa(&self) -> bool { self.0 == CODE_ID_NONE }
    pub fn as_u32(&self) -> u32 { self.0 }
    pub fn to_le_bytes(&self) -> [u8; CODE_ID_SIZE] { self.0.to_le_bytes() }
...
```bash
git add src/schema40.rs
jj describe -m "feat: add schema40 v3 constants and tag/code id"
```

---

### Task 2: Implement AccountEntry40 (v3 layout)
...
Run: `cargo test schema40::tests::test_account_entry_roundtrip`
Expected: FAIL (missing AccountEntry40).

**Step 3: Implement AccountEntry40**
...
impl AccountEntry40 {
    pub fn new(balance_256: &[u8; 32], nonce: u64, code_id: CodeId, address: &[u8; 20]) -> Self {
        ...
        let nonce_u32 = if nonce > u32::MAX as u64 { u32::MAX } else { nonce as u32 };
        Self {
            balance,
            nonce: nonce_u32,
            code_id,
    ...
    pub fn from_bytes(bytes: &[u8; ENTRY_SIZE]) -> Self {
        ...
        Self { balance, nonce, code_id, tag, _padding: [0u8; ACCOUNT_PADDING_SIZE] }
    }

    pub fn nonce_u64(&self) -> u64 { self.nonce as u64 }
}
```
...
```bash
git add src/schema40.rs
jj describe -m "feat: add schema40 v3 account entry"
```

---

### Task 3: Implement StorageEntry40 (v3 layout)
...
Run: `cargo test schema40::tests::test_storage_entry_roundtrip`
Expected: FAIL (missing StorageEntry40).

**Step 3: Implement StorageEntry40**
...
impl StorageEntry40 {
    pub fn new(value: &[u8; 32], address: &[u8; 20], slot_key: &[u8; 32]) -> Self {
        Self { value: *value, tag: Tag::from_address_slot(address, slot_key) }
    }
...
```bash
git add src/schema40.rs
jj describe -m "feat: add schema40 v3 storage entry"
```

---

### Task 4: Add CodeStore (bytecode hash dedup)
...
Run: `cargo test schema40::tests::test_code_store_basic`
Expected: FAIL (missing CodeStore).
...
impl CodeStore {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        if bytes.len() != expected {
            return Err(format!("Code store size mismatch: expected {}, got {}", expected, bytes.len()));
        }
...
```bash
git add src/schema40.rs
jj describe -m "feat: add schema40 code store"
```

---

### Task 5: Full schema40 test sweep

**Files:**
- Test: `src/schema40.rs`

**Step 1: Run schema40 tests**

Run: `cargo test schema40::tests`
Expected: PASS
...
```bash
git add src/schema40.rs
jj describe -m "test: finalize schema40 v3"
```

---

Plan complete and saved to `docs/plans/2026-01-30-schema40-v3-codestore-implementation.md`. Two execution options:
