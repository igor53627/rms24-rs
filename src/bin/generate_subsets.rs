...
fn main() -> io::Result<()> {
    ...
    info!("RMS24 Subset Generator (Phase 1)");
    ...
    info!("Database: {} ({:.2} GB, {} entries)", args.db, db_size as f64 / 1e9, num_entries);
    ...
    info!("Generating: {} subsets", total_hints);
    ...
    info!("Subset generation: {:.2}s", phase1_time.as_secs_f64());

    let subset_data = SubsetData::from_subsets(&subsets);
    info!("Total blocks in subsets: {}", subset_data.blocks.len());
    ...
    info!("Saved: {:.2} MB", file_size as f64 / 1e6);
